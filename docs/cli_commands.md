...
**Filtering only applies when enabled** (`filter on`) and never affects:
- Directly-routed packets (only flood packets are filtered).
- Packets to or from a contact in the node's [ACL](#acl) (these are always prioritized for `REQ`, `RESPONSE`, `TXT_MSG`, `ANON_REQ` and `PATH` payload types).

Many subcommands act on a **payload type**, identified by an index. Use `filter types` to list them:
...
### Show blocked-packet summary
**Usage:** `filter`

**Note:** Shows whether the filter is on/off and total packets blocked, broken down by reason: `Hops`, `Rate`, `Channel`, `Hash` and `Malformed`. Counters are cleared by [`clear stats`](#clear-stats).
...
### Reset filter preferences to defaults
**Usage:** `filter reset`

**Warning:** Restores all filter settings (hops, rates, channels, hash size, malformed scan) to their defaults.

---

### List payload type indices
**Usage:** `filter types`

---

### Show per-type blocked counts
**Usage:** `filter count`

**Note:** Lists `HOPS,RATE` blocked counts per payload type.

---

### Maximum hops per payload type
**Usage:**
- `filter hops` — list the current max hops for every payload type
- `filter hops <type> <max_hops>`

**Parameters:**
- `type`: Payload type index (`0`–`11`)
- `max_hops`: Maximum allowed hop count, `0`–`64`. Flood packets at or above this hop count for the given type are dropped.

---

### Rate limit per payload type
**Usage:**
- `filter rate` — list the current rate limit for every payload type
- `filter rate <type> <limit> <secs>`

**Parameters:**
- `type`: Payload type index (`0`–`11`)
- `limit`: Maximum number of packets allowed within the window
- `secs`: Window length in seconds. Packets of the given type beyond `limit` per `secs` are dropped.
