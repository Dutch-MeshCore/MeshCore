...
static const size_t kV1PreConfigPayloadSize = MQTT_PREFS_V1_PRE_CONFIG_PAYLOAD_SIZE;
...
inline size_t payloadLenFor(const MQTTPrefs& prefs) {
  ...
  // round-trips: the mqtt.config flag needs FULL; else a non-default filter-stats
  // interval needs PRE_CONFIG; else a non-default mask needs PRE_FILTERSTATS;
  // otherwise PRE_FILTER.
  ...
  if (prefs.mqtt_config_enabled != 0) return kV1BaselinePayloadSize;
  if (!interval_default) return kV1PreConfigPayloadSize;
...
inline void freezeV1(const MQTTPrefs& prefs, LegacyV1MQTTPrefs* frozen) {
  ...
  frozen->mqtt_filter_interval = prefs.mqtt_filter_interval;
  frozen->mqtt_config_enabled = prefs.mqtt_config_enabled;
}
...
inline DecodePlan classify(const uint8_t* prefix, size_t prefix_read, size_t file_size) {
  ...
  if (file_size >= sizeof(MQTTPrefsHeader)) {
    ...
    if (memcmp(header.magic, MQTT_PREFS_MAGIC, sizeof(header.magic)) == 0) {
      ...
      if (header.payload_len == kV1PreConfigPayloadSize) {
        // Written before the mqtt.config flag tail. Every earlier field (incl. the
        // filter-stats interval) is present; only the flag is missing, so it keeps
        // its default (off).
        return {Source::Current, false, false, true, kV1PreConfigPayloadSize};
      }
...
inline void migrateV1(const LegacyV1MQTTPrefs& old_prefs, size_t payload_len,
  ...
  if (payload_len >= kV1PreFilterStatsPayloadSize) {
    memcpy(prefs->mqtt_slot_packet_filter, old_prefs.mqtt_slot_packet_filter,
           sizeof(prefs->mqtt_slot_packet_filter));
  }
  if (payload_len >= kV1PreConfigPayloadSize) {
    prefs->mqtt_filter_interval = old_prefs.mqtt_filter_interval;
  }
