...
class MQTTPrefsSerializer : public ConfigSerializer {
  ...
  // DMC packet-filter drop statistics. Only the publish interval is persisted;
  // 0 disables the filter-stats topic, any other value is clamped into the
  // 60s-10min band the CLI enforces (set mqtt.filter.interval).
  class FilterStatsPrefs : public ConfigSerializer {
    MQTTPrefs* _prefs;
    int32_t _interval_ms;
    bool _seen_interval = false;
  protected:
    void structure() override {
      defStrict("interval_ms", _interval_ms, _seen_interval);
    }
  public:
    explicit FilterStatsPrefs(MQTTPrefs* prefs)
        : _prefs(prefs), _interval_ms(static_cast<int32_t>(prefs->mqtt_filter_interval)) {}
    void apply(bool* repaired) {
      if (_interval_ms != 0 &&
          (_interval_ms < static_cast<int32_t>(MQTT_FILTER_STATS_MIN_INTERVAL_MS) ||
           _interval_ms > static_cast<int32_t>(MQTT_FILTER_STATS_MAX_INTERVAL_MS))) {
        _interval_ms = static_cast<int32_t>(MQTT_FILTER_STATS_DEFAULT_INTERVAL_MS);
        *repaired = true;
      }
      _prefs->mqtt_filter_interval = static_cast<uint32_t>(_interval_ms);
    }
  };
  ...
  class MqttPrefs : public ConfigSerializer {
    ...
    int32_t _config_enabled;   // DMC opt-in `config` topic (set mqtt.config)
    ...
    bool _seen_config = false;
    StatusPrefs _status;
    NeighborPrefs _neighbors;
    FilterStatsPrefs _filter_stats;
    OwnerPrefs _owner;
  ...
  protected:
    void structure() override {
      ...
      defStrict("config_enabled", _config_enabled, _seen_config);
      def("status", _status);
      def("neighbors", _neighbors);
      def("filter_stats", _filter_stats);
      def("owner", _owner);
      def("slot1", _slot1);
      def("slot2", _slot2);
      def("slot3", _slot3);
  ...
  public:
    MqttPrefs(MQTTPrefs* prefs, const MQTTPrefs* defaults)
        : _prefs(prefs), _packets_enabled(prefs->mqtt_packets_enabled),
          ...
          _rx_enabled(prefs->mqtt_rx_enabled),
          _config_enabled(prefs->mqtt_config_enabled),
          _status(prefs), _neighbors(prefs), _filter_stats(prefs),
    ...
    void apply(bool* repaired) {
      ...
      if (_config_enabled < 0 || _config_enabled > 1) { _config_enabled = 0; *repaired = true; }
      ...
      _prefs->mqtt_config_enabled = static_cast<uint8_t>(_config_enabled);
